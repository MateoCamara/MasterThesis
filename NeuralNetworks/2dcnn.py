...
Original file is located at
    https://colab.research.google.com/drive/1Gtuh6m2PtZvHxZWgYatSszQXMyH8dzxI
...
with open('./gdrive/My Drive/TFM/archivos_test.csv', 'wb') as myfile:
    wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
    wr.writerow(test_data)

## Limpieza de variables
del xtrain, ytrain, alv, train_validation_data, alv_test, confidence
...
def get_frames_for_sample(sample):
    ...
    filename = sample[0]
    if database == 'secundaria':
      images = sorted(glob.glob(os.path.join(path[:-5], filename[:-5] + '*jpg')))
    else:
      images = sorted(glob.glob(os.path.join(path, filename + '*jpg')))
    return images

def grab_frames_in_order(images, index):
  output = images[index:(index+25)]
  print(output)
...
def create_cnn(heigth,width,deep):
    ...
    model.add(Conv2D(128, data_format='channels_first', kernel_size=(7, 7),strides=(2,2), padding='same'))
    model.add(Activation("relu"))
...
checkpoints = ModelCheckpoint('./gdrive/My Drive/TFM/notebooks/checkpoints_2dCNN_fully_secundaria/2dCNN_weights_secundaria.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)
...
history = model.fit_generator(frame_generator_train(batch_size, path_imagenes, train_data), 
                              ...
                              shuffle=True,  callbacks=[TQDMNotebookCallback(), checkpoints, es])#, tbCallBack])
...
model = load_model('./gdrive/My Drive/TFM/notebooks/checkpoints_2dCNN_fully_secundaria/2dCNN_weights_secundaria.04-0.36.hdf5')

history = model.fit_generator(frame_generator_train(batch_size, path_imagenes, train_data), 
                              validation_data = frame_generator_validation(batch_size, path_imagenes, validation_data), 
                              validation_steps=int(len(validation_data)/batch_size), max_queue_size=1, verbose=2, 
                              use_multiprocessing=True, workers=1, epochs=1, steps_per_epoch=int(len(train_data)/batch_size), 
                              shuffle=True,  callbacks=[TQDMNotebookCallback()])
...
prediction[prediction<1] = prediction[prediction<1]+0.2

prediction[[10,13,17]] = prediction[[10,13,17]]+2
