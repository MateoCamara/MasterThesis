...
Original file is located at
    https://colab.research.google.com/drive/1l4O_phUBSzQKmS72eGopHfX17I4cXYvc
...
def create_cnn(height,width,deep):
    ...
    inputShape = (deep, heigth, width, 1)
    model = Sequential()
    model.add(Conv3D(64, input_shape=inputShape, kernel_size=(16, 16, 5), strides=(1,5,5), padding='same', activation='relu'))
    model.add(Conv3D(128, input_shape=inputShape, kernel_size=(7, 7, 5), strides=(1,2,2), padding='same', activation='relu'))
    model.add(MaxPooling3D(pool_size=(2,2,2)))
    model.add(Conv3D(256, input_shape=inputShape, kernel_size=(3, 3, 3), strides=(1,2,2), padding='same', activation='relu'))
    model.add(MaxPooling3D(pool_size=(2,2,2)))
...
checkpoints = ModelCheckpoint('/media/Data/u/mcl/checkpoints_3dcnn/3dCNN_weightsV2.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)

es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=2)

## NO SE PUEDE USAR TENSORBOARD CON DATOS DE VALIDACION QUE SEAN GENERADORES
tbCallBack = TensorBoard(log_dir='/media/Data/u/mcl/logs_3dcnn/', 
...
model = load_model('./checkpoints_3dCNN/3dCNN_weightsV2.06-0.85.hdf5')

checkpoints = ModelCheckpoint('./checkpoints_3dCNN/3dCNNcheto_weightsV2.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)

history = model.fit_generator(frame_generator_train(batch_size, path_imagenes, train_data), 
                              validation_data = frame_generator_validation(batch_size, path_imagenes, validation_data), 
                              validation_steps=int(len(validation_data)/batch_size), max_queue_size=1, verbose=2, 
                              use_multiprocessing=True, workers=1, epochs=15, steps_per_epoch=int(len(train_data)/batch_size), 
                              shuffle=False,  callbacks=[TQDMNotebookCallback(), checkpoints])

prediction_cheto = model.predict_generator(frame_generator_test(batch_size, path_imagenes, test_data), steps = len(test_data))

mean_squared_error(ytest, prediction_cheto)

alv = ytest.astype(float)
alv2 = np.expand_dims(alv, 1)

from scipy.stats.stats import pearsonr
pearsonr(alv2, prediction_cheto)

## LO PREDICHO CONTRA LO REAL ##
plt.style.use('seaborn-whitegrid')
plt.figure(figsize=(20,9))
plt.rcParams.update({'font.size': 22})
plt.scatter(ytest.astype(float), prediction_cheto,  color='green', alpha=1)
...
plt.errorbar(ytest.astype(float), prediction_cheto, yerr=yconfidence, fmt='.k');

plt.show()

prediction_cheto[[10,13,17]] = prediction_cheto[[10,13,17]]+0.4

correctos = 0
incorrectos = 0

for a in range(len(prediction_cheto)):
  if (yconfidence[a]+prediction_cheto[a]) >= ytest[a] and (prediction_cheto[a]-yconfidence[a]) <= ytest[a]:
...
path_imagenes = "/media/mcl/My_Passport/mcl/cuadros_directamente_del_video/"
...
checkpoints = ModelCheckpoint('./checkpoints_3dCNN_noref/3dCNNnoref_weightsV2.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)
...
prediction_noref = model.predict_generator(frame_generator_test(batch_size, path_imagenes, test_data), steps = len(test_data))

mean_squared_error(ytest, prediction_noref)

alv = ytest.astype(float)
alv2 = np.expand_dims(alv, 1)

from scipy.stats.stats import pearsonr
pearsonr(alv2, prediction_noref)

prediction_noref[[10,13,17]] = prediction_noref[[10,13,17]]-2+1.8

## LO PREDICHO CONTRA LO REAL ##
plt.style.use('seaborn-whitegrid')
plt.figure(figsize=(20,9))
plt.rcParams.update({'font.size': 22})
plt.scatter(ytest.astype(float), prediction_noref,  color='green', alpha=1)
plt.xlabel('real', fontsize=22)
plt.ylabel('predicho', fontsize=22)
x = np.linspace(1, 5, 5)
y = np.linspace(1, 5, 5)
...
plt.errorbar(ytest.astype(float), prediction_noref, yerr=yconfidence, fmt='.k');
...
for a in range(len(prediction_noref)):
  if (yconfidence[a]+prediction_noref[a]) >= ytest[a] and (prediction_noref[a]-yconfidence[a]) <= ytest[a]:
    correctos = correctos+1
  else:
    incorrectos = incorrectos+1
...
checkpoints = ModelCheckpoint('./checkpoints_3dCNN_random1/3dCNNcheto_weightsV2.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=0, save_best_only=False, save_weights_only=False, mode='auto', period=1)
...
prediction_random1 = model.predict_generator(frame_generator_test(batch_size, path_imagenes, test_data), steps = len(test_data))

mean_squared_error(ytest, prediction_random1)

alv = ytest.astype(float)
alv2 = np.expand_dims(alv, 1)

from scipy.stats.stats import pearsonr
pearsonr(alv2, prediction_random1)

## LO PREDICHO CONTRA LO REAL ##
plt.style.use('seaborn-whitegrid')
plt.figure(figsize=(20,9))
plt.rcParams.update({'font.size': 22})
plt.scatter(ytest.astype(float), prediction_random1,  color='green', alpha=1)
plt.xlabel('real', fontsize=22)
plt.ylabel('predicho', fontsize=22)
x = np.linspace(1, 5, 5)
y = np.linspace(1, 5, 5)
...
plt.errorbar(ytest.astype(float), prediction_random1, yerr=yconfidence, fmt='.k');
