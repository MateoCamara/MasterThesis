# -*- coding: utf-8 -*-
"""LSTM_truncated.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1g_8Iv7Hi4hem3v2V1AWO2ckiRbOwnl7L
...
def create_cnn(height,width, deep):
    ...
    inputShape = (248, height, width, deep)
    model = Sequential()

    model.add(TimeDistributed(Conv2D(32, (7, 7), strides=(2, 2),
        activation='relu', padding='same'), input_shape=inputShape, batch_input_shape=(1,248,height, width, deep)))
    ...
    model.add(Reshape((1, 45056)))

    model.add(LSTM(248, return_sequences=False, stateful=True))
...
def create_cnn(rows, columns, deep):
    # initialize the input shape and channel dimension, assuming
    # TensorFlow/channels-last ordering
    inputShape = (1, rows, columns, deep)
    model = Sequential()

    model.add(Conv2D(32, (7, 7), strides=(2, 2),
        activation='relu', padding='same', input_shape=inputShape, data_format="channels_last", batch_input_shape=[1,rows, columns, deep]))#, batch_input_shape=[1,1,1,1]))
...
def create_cnn(rows, columns, deep):
    # initialize the input shape and channel dimension, assuming
    # TensorFlow/channels-last ordering
    inputShape = (None, rows, columns, deep)
    model = Sequential()

    model.add(TimeDistributed(Conv2D(64, (32, 32), strides=(32,32),
        activation='relu', padding='same', input_shape=inputShape, batch_input_shape=[None,rows, columns, deep])))#, batch_input_shape=[1,1,1,1]))
    model.add(TimeDistributed(Flatten()))
    model.add(TimeDistributed(Reshape((1, int(248/4), 6912))))

    model.add(LSTM(248, return_sequences=False, stateful=True))
    model.add(Dense(1, activation='linear'))
    
    adam = Adam(lr=0.0000001, decay=1e-6)
    model.compile(loss='mean_squared_error', optimizer=adam)
    return model
...
def create_cnn(rows,columns, deep):
    # initialize the input shape and channel dimension, assuming
    # TensorFlow/channels-last ordering
    inputShape = (1, deep, rows, columns)
    model = Sequential()

    model.add(TimeDistributed(Conv2D(32, (16, 16), strides=(3,3),
        activation='relu', data_format='channels_first', padding='same'), input_shape=inputShape, batch_input_shape=[1, 1,deep, rows, columns]))
    model.add(TimeDistributed(Conv2D(64, (7,7), strides=(2,2),
        kernel_initializer="he_normal", activation='relu', data_format='channels_first' )))
    model.add(TimeDistributed(MaxPooling2D((2, 2), data_format='channels_first' )))
    model.add(TimeDistributed(Conv2D(128, (3,3), strides=(2, 2),
    ...
    model.add(LSTM(64, return_sequences=True, dropout=0.1, stateful=True))
    model.add(LSTM(32, return_sequences=False, dropout=0.1, stateful=True))
    model.add(Dense(1, activation='linear'))
...
model = create_cnn(rows, columns, 20)
...
for epoch in tqdm(range(10)):
    ...
    for i in tqdm(range(int(len(train_data)))): #len(train_data)
      ...
      for image_id in tqdm(np.linspace(0, 220, 12)):
        ...
        tr_loss = model.train_on_batch(np.expand_dims(np.expand_dims(np.array(sequence), axis=1), axis=0), np.array([y_true]))
        #mean_tr_acc.append(tr_acc)
        mean_tr_loss.append(tr_loss)
        print('\r', "Error en la secuencia "+ sample +": " + str(tr_loss), end='')
        if tr_loss < 0.00001:
          print('\n Detectado sobreajuste en está secuencia, c sigue.')
          break
...
'''
    for i in tqdm(range(int(len(test_data)/30))):
      ...
      for image_id in tqdm(range(248)):
        ...
        te_loss = model.test_on_batch(np.array(frame_processed), np.array([y_true]))
        #mean_tr_acc.append(tr_acc)
...
model.save('./gdrive/My Drive/TFM/notebooks/checkpoints_LSTM_truncated/modelo_final.h5')
'''
